...
<form id="submitReorder" action="{if isset($opc) && $opc}{$link->getPageLink('order-opc', true)}{else}{$link->getPageLink('order', true)}{/if}" method="post" class="submit">
		...
		<input type="hidden" value="" name="submitReorder"/>
			...
			<a href="#" onclick="$(this).closest('form').submit(); return false;" class="button btn btn-default button-medium pull-right"><span>{l s='Reorder'}<i class="fa fa-angle-right right"></i></span></a>
		<p class="dark">
				...
				{if isset($product.customizedDatas)}
					<tr class="item">
						...
						<td>
							<div class="input-group quantity-control" unselectable="on" style="-moz-user-select: none;">
								<a href="#" class="return_quantity_down input-group-addon cart_quantity_down disabled">-</a>ss							
								<input class="order_qte_input cart_quantity_input form-control grey"  name="order_qte_input[{$smarty.foreach.products.index}]" type="text" size="2" value="{$product.customizationQuantityTotal|intval}" />																
								<a href="#" class="return_quantity_up input-group-addon cart_quantity_up disabled">+</a>
							</div>                        
					...
					{foreach $product.customizedDatas  as $customizationPerAddress}
						...
						<tr class="alternate_item">
							...
							<td>
								<div class="input-group quantity-control" unselectable="on" style="-moz-user-select: none;">
									<a href="#" class="return_quantity_down input-group-addon cart_quantity_down disabled">-</a>							
									<input class="order_qte_input cart_quantity_input form-control grey"  name="customization_qty_input[{$customizationId|intval}]" type="text" size="2" value="{$customization.quantity|intval}" />																
									<a href="#" class="return_quantity_up input-group-addon cart_quantity_up disabled">+</a>
								</div> 
				...
				{if $product.product_quantity > $product.customizationQuantityTotal}
					<tr class="item">
						...
						<td class="return_quantity">
							<div class="input-group quantity-control" unselectable="on" style="-moz-user-select: none;">
									<a href="#" class="return_quantity_down input-group-addon cart_quantity_down disabled">-</a>							
									<input class="order_qte_input cart_quantity_input form-control grey"  name="order_qte_input[{$product.id_order_detail|intval}]" type="text" size="2" value="{$productQuantity|intval}" />																
									<a href="#" class="return_quantity_up input-group-addon cart_quantity_up disabled">+</a>
							</div>							
...
{if $return_allowed}
	<div id="returnOrderMessage">
		...
		<p class="form-group">
			<button type="submit" name="submitReturnMerchandise" class="btn btn-default button button-small"><span>{l s='Make an RMA slip'}<i class="fa fa-single-right right"></i></span></button>
...
{if !$is_guest}
	...
	<form action="{$link->getPageLink('order-detail', true)|escape:'html':'UTF-8'}" method="post" class="std" id="sendOrderMessage">
		...
		<div class="submit">
			...
			<button type="submit" name="submitMessage" class="button btn btn-default"><span>{l s='Send'}<span class="fa fa-single-right right"></span></span></button>
		</div>
	</form>
{else}
<p class="alert alert-info"><i class="fa fa-info"></i>{l s='You cannot return merchandise with a guest account'}</p>
{/if}
{/if}