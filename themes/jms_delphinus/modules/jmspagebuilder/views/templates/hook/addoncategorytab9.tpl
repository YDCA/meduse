...
jQuery(function ($) {
    "use strict";
	var categorytabCarousel = $(".categorytab-carousel1");			
...
<div class="jms-tab">
	<ul class="nav nav-tabs" role="tablist">
		{foreach from = $categories key = k item = category}
			<li class="{if $k == 0}active{/if}"><a class="button" data-toggle="tab" href="#category-{$category.id_category|escape:'html':'UTF-8'}">{$category.name}</a></li>
		{/foreach}	
	</ul>
</div>
<div class="tab-content row">
	{foreach from = $categories key = k item = category}
		 <div role="tabpanel" class="tab-pane {if $k == 0}active{/if}" id="category-{$category.id_category|escape:'html':'UTF-8'}">
			<div class="categorytab-carousel1">	
							...
							<div class="product-box product-preview" itemscope itemtype="http://schema.org/Product">
								<div class="preview">
									...
									<div class="product_action">
										...
										{/if}
										<a data-link="{$product.link|escape:'html':'UTF-8'}" class="quick-view btn-radius-square product-btn hidden-xs" title="{l s='Quick View' mod='jmsproductfilter'}">
									...
									<div class="action">
										{if ($product.id_product_attribute == 0 OR (isset($add_prod_display) AND ($add_prod_display == 1))) AND $product.available_for_order AND !isset($restricted_country_mode) AND $product.minimal_quantity == 1 AND $product.customizable != 2 AND !$PS_CATALOG_MODE}
											...
											<a class="product-btn cart-button btn-default active ajax_add_to_cart_button" data-id-product="{$product.id_product}" href="{$link->getPageLink('cart')|escape:'html':'UTF-8'}?qty=1&amp;id_product={$product.id_product}&amp;token={$static_token}&amp;add" title="{l s='Add to cart' mod='jmsproductfilter'}">
