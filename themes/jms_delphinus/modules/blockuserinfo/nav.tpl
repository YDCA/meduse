...
<div class="btn-group compact-hidden user-info">
	{if $is_logged}
		<a data-toggle="dropdown" class="btn-xs dropdown-toggle login account" href="{$link->getPageLink('my-account', true)}" title="{l s='View my customer account' mod='blockuserinfo'}" rel="nofollow">
	...
	{else}
		<a data-toggle="dropdown" class="btn-xs dropdown-toggle login" href="{$link->getPageLink('my-account', true)}" title="{l s='Login to your customer account' mod='blockuserinfo'}" rel="nofollow">
				<span class="text-box">{l s='Account' mod='blockuserinfo'}</span>
		</a>
	{/if}
