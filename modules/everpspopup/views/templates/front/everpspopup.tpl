...
<div id="everpspopup_block_center" data-delay="{$everpspopup->delay|escape:'htmlall':'UTF-8'}" data-adult="{$everpspopup->adult_mode|escape:'htmlall':'UTF-8'}" data-expire="{$everpspopup->cookie_time|escape:'htmlall':'UTF-8'}" class="Everpopup_block" style="display:none;{if $background}background-image: url({$link->getMediaLink($background)|escape:'htmlall':'UTF-8'});background-size:cover;{/if}">
    {if $everpspopup->link}<a href="{$everpspopup->link|escape:'htmlall':'UTF-8'}" rel="nofollow">{/if}
        <div class="container"{if $everpspopup->bgcolor} style="background-color:{$everpspopup->bgcolor|escape:'htmlall':'UTF-8'};"{/if}>
            <div class="row">
                {if $content}<div class="rte col-xs-12">{$content nofilter}</div>{/if}
            </div>
        </div>
