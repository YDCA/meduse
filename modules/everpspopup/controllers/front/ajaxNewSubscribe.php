        ...
        if ($ps_activeNewsletter) {
            if (empty($_SERVER['REMOTE_ADDR']) || !filter_var($_SERVER['REMOTE_ADDR'], FILTER_VALIDATE_IP)) {
                die(Tools::jsonEncode(array(
                    'return' => false,
                    'error' => $this->module->l('User ip not found or not valid')
                )));
            }
            ...
            if (empty(Tools::getValue('ever_email')) || !Validate::isEmail(Tools::getValue('ever_email'))) {
                die(Tools::jsonEncode(array(
                    'return' => false,
                    'error' => $this->module->l('Mail address is empty or is not valid.')
                )));
            }
            ...
            if ($subscribed) {
                ...
                if (!$activate) {
                    die(Tools::jsonEncode(array(
                        'return' => false,
                        'error' => $this->module->l('Error : Can\'t update the newsletter subscription')
                    )));
                }

                die(Tools::jsonEncode(array(
                    'return' => true,
                    'message' => $this->module->l('You\'ve already registered to our mailing list')
                )));
            }
            ...
            if ($newSubscription) {
                die(Tools::jsonEncode(array(
                    'return' => true,
                    'message' => $this->module->l('Thank you ! Your e-mail has been successfuly registered.')
                )));
            }
            ...
            die(Tools::jsonEncode(array(
                'return' => true,
                'message' => $this->module->l('Module Newsletter not activated')
            )));
        }
    }
}
