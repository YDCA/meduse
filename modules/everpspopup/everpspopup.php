...
class Everpspopup extends Module
{
            ...
            'form' => array(
                ...
                'input' => array(
                    array(
                        'col' => 3,
                        'type' => 'text',
                        'desc' => $this->l('Minimum legal age'),
                        'name' => 'EVERPSPOPUP_AGE',
                        'label' => $this->l('For adult mode'),
                        'required' => 'true',
                    ),
                    array(
                        'type' => 'switch',
    ...
    public function hookHeader()
    {
        if ((int)Configuration::get('EVERPSPOPUP_FANCYBOX')) {
            ...
            $this->context->controller->addJS(($this->_path).'views/js/fancybox.js', 'all');
        }
    ...
    */
    public function hookDisplayBeforeBodyClosingTag()
    {
        ...
        if (file_exists(self::POPUP_IMG.'everpopup_'.(int)$everpopup->id.'.jpg')) {
            $background = $this->_path.'views/img/everpopup_'.(int)$everpopup->id.'.jpg';
        } else {
            $background = false;
        }
        $this->smarty->assign(
