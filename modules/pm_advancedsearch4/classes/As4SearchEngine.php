...
abstract class As4SearchEngine
{
    ...
    protected static $pm_spa_is_active = null;
    protected static $pm_spa_hide_color_squares = null;
    ...
    public static function generateURLFromCriterions($idSearch, $criterionsList = array(), $idLang = null, $array = array(), $pagination = false)
    {
        ...
        if (!sizeof($criterionsList) && !$forceContextUrlGeneration) {
            if ($isFromHome) {
                ...
                if (Tools::getValue('productFilterListSource') == 'jolisearch') {
                    return $context->link->getModuleLink('ambjolisearch', 'jolisearch', array('search_query' => Tools::replaceAccentedChars(urldecode(self::$productFilterListData))));
                } elseif (Tools::getValue('productFilterListSource') == 'module-ambjolisearch-jolisearch') {
                    return $context->link->getModuleLink('ambjolisearch', 'jolisearch', array('s' => Tools::replaceAccentedChars(urldecode(self::$productFilterListData))));
                } elseif (isset($pagesList[Tools::getValue('productFilterListSource')])) {
                    if (Tools::getValue('productFilterListSource') == 'search') {
                        return $context->link->getPageLink(Tools::getValue('productFilterListSource'), null, null, array('search_query' => Tools::replaceAccentedChars(urldecode(self::$productFilterListData))));
                    } else {
        ...
        if (self::$productFilterListSource) {
            ...
            if (self::$productFilterListSource == 'search' || self::$productFilterListSource == 'jolisearch') {
                $params['search_query'] = Tools::replaceAccentedChars(urldecode(self::$productFilterListData));
            } elseif (self::$productFilterListSource == 'module-ambjolisearch-jolisearch') {
                $params['s'] = Tools::replaceAccentedChars(urldecode(self::$productFilterListData));
            }
        }
    ...
    {
        if (self::$pm_spa_is_active === null) {
            $module = Module::getInstanceByName('pm_productsbyattributes');
            if (is_object($module) && $module->active) {
                self::$pm_spa_is_active = true;
            } else {
                self::$pm_spa_is_active = false;
            }
        }
        return self::$pm_spa_is_active;
    }
