...
class PM_AdvancedSearch4 extends AdvancedSearchWidgetProxy
{
                    ...
                    if (Validate::isLoadedObject($seo_search) && isset($seo_search->criteria) && !empty($seo_search->criteria)) {
                        ...
                        if (self::_isFilledArray($criteres_seo)) {
                            foreach ($criteres_seo as $critere_seo) {
                                $critere_seo = explode('_', $critere_seo);
                                $id_criterion_group_seo = (int)$critere_seo[0];
                                if (Tools::strpos($critere_seo[1], '-') === false) {
    ...
    public function putInSmartyCache($cacheId, $data)
    {
        ...
        if (!empty($config['moduleCache'])) {
            ...
            $this->display(__FILE__, $this->templatePrefix . 'cache.tpl', $cacheId);
        }
    }
    public function getFromSmartyCache($cacheId)
    {
        ...
        if (!empty($config['moduleCache'])) {
            ...
            if ($this->isCached($templatePath, $cacheId)) {
                if (version_compare(_PS_VERSION_, '1.7.0.0', '>=')) {
                    $cacheData = unserialize(self::getDataUnserialized(trim(strip_tags($this->display(__FILE__, $templatePath, $cacheId)))));
                } else {
    ...
    public function includeAssets()
    {
        ...
        if (!Tools::getValue('ajaxMode')) {
            if (version_compare(_PS_VERSION_, '1.7.0.0', '<')) {
                ...
                $chosenInclusionsList = Media::getJqueryPluginPath('chosen', _PS_JS_DIR_.'jquery/plugins/chosen/');
                if (self::_isFilledArray($chosenInclusionsList)) {
                    if (!empty($chosenInclusionsList['js'])) {
                        $this->context->controller->removeJS($chosenInclusionsList['js']);
                    }
                    if (self::_isFilledArray($chosenInclusionsList['css'])) {
                        foreach ($chosenInclusionsList['css'] as $cssUri => $mediaType) {
                            $this->context->controller->removeCSS($cssUri, $mediaType);
                        }
                    }
                }
                $this->context->controller->addJS(__PS_BASE_URI__ . 'modules/' . $this->name . '/views/js/chosen/chosen.jquery.min.js');
                $this->context->controller->addCSS(__PS_BASE_URI__ . 'modules/' . $this->name . '/views/css/chosen/chosen.min.css', 'all');
                ...
                if (self::_isFilledArray($chosenInclusionsList)) {
                    ...
                    if (self::_isFilledArray($chosenInclusionsList['css'])) {
                        foreach ($chosenInclusionsList['css'] as $cssUri => $mediaType) {
                            $this->context->controller->unregisterStylesheet('chosen/jquery.chosen.css');
                        }
                    }
                }
                $this->context->controller->registerJavascript('chosen/jquery.chosen.js', 'modules/'.$this->name.'/views/js/chosen/chosen.jquery.min.js', array('position' => 'bottom', 'priority' => 150));
                $this->context->controller->registerStylesheet('chosen/jquery.chosen.css', 'modules/'.$this->name.'/views/css/chosen/chosen.min.css', array('media' => 'all', 'priority' => 900));
    ...
    {
        if (get_class($this->context->controller) == 'AdminProductsController') {
            return;
        }
    ...
    public function hookActionAdminProductsControllerSaveAfter($params)
    {
        ...
        if (!empty($conf['autoReindex']) && isset($params['return']) && Validate::isLoadedObject($params['return'])) {
            As4SearchEngineIndexation::$processingAutoReindex = true;
            As4SearchEngineIndexation::indexCriterionsFromProduct($params['return']);
            As4SearchEngineIndexation::$processingAutoReindex = false;
        }
    }
