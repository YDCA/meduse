...
class pm_crosssellingoncart extends Module
{
    ...
    public function __construct($prefixFieldsOptions = 'PM_CSOC')
    {
        ...
        $this->displayName = 'Cross Selling On Cart';
    ...
    public function setDefaultConfiguration()
    {
        ...
        if (version_compare(_PS_VERSION_, '1.7.0.0', '>=')) {
            ...
            Configuration::updateValue('PM_' . self::$modulePrefix . '_IMAGE_SIZE', 'home' . '_' . 'default');
        } else {
            ...
            Configuration::updateValue('PM_' . self::$modulePrefix . '_IMAGE_SIZE', 'medium' . '_' . 'default');
        }
    ...
    public function postProcess()
    {
        ...
        foreach (array_keys($formValues) as $key) {
            ...
            } elseif ($key == 'PM_' . self::$modulePrefix . '_CROSSSELLING_NB_DAYS') {
                Configuration::updateValue($key, min(max(1, Tools::getValue($key)), 200));
    ...
    private function checkIfModuleIsUpdate($updateDb = false, $displayConfirm = true)
    {
        ...
        if (!$this->isRegisteredInHook('displayShoppingCartFooter')) {
            $this->registerHook('displayShoppingCartFooter');
        }
        if (!$updateDb && $this->version != Configuration::get('PM_' . self::$modulePrefix . '_LAST_VERSION')) {
            return false;
        }
        if ($updateDb) {
        ...
        $configForm = array(
            'form' => array(
                ...
                'input' => array(
                    ...
                    array(
                        'type' => 'html',
                        'html_content' => '<h2>' . $this->l('Advanced Styles (CSS)') . '</h2>',
    ...
    protected function getAccessoriesLight($productList, $limit, $ignoreList = array())
    {
    ...
    protected function getCrossSellingLight($productList, $limit, $nbDays)
    {
        if (!self::isFilledArray($productList)) {
            return false;
        }
        return Db::getInstance(_PS_USE_SQL_SLAVE_)->ExecuteS('
			...
			WHERE od.product_id NOT IN ('.implode(',', array_map('intval', $productList)).') AND od.id_order IN (
				...
				WHERE o.valid = 1 AND od.product_id IN ('.implode(',', array_map('intval', $productList)).') AND TO_DAYS("'.date('Y-m-d').' 00:00:00") - TO_DAYS(o.date_add) <= '.$nbDays.'
			)
		 	AND ctg.id_group IS NOT NULL
			LIMIT '.(int)$limit);
    }
    ...
    public function hookDisplayShoppingCartFooter($params)
    {
        ...
        $nbDays = Configuration::get($this->prefixFieldsOptions.'_CROSSSELLING_NB_DAYS');
        ...
        if (Configuration::get('PM_' . self::$modulePrefix . '_CROSSSELLING')) {
            $productsCrossSelling = $this->getCrossSellingLight($productsOnCart, $nbCrossSelling, $nbDays);
