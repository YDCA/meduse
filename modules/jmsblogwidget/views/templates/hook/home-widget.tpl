...
jQuery(function ($) {
	...
	var items = {$widget_setting.JBW_ITEM_SHOW|escape:'htmlall':'UTF-8'},		
	    itemsDesktop = {$widget_setting.JBW_ITEM_SHOW|escape:'htmlall':'UTF-8'},
	    itemsDesktopSmall = 2,
	    itemsTablet = 1,
	    itemsMobile = 1;	
	$(this).owlCarousel({
...
<div class="jmsblog-home-widget">
	<div class="block-title">
		<h3>
			<span>{l s='Latest Post' mod='jmsblogwidget'}</span>
		</h3>
	</div>
	{if $posts|@count gt 0}
	...
	{foreach from=$posts item=post}		
		...
		<div class="blog-item">
			...
			<div class="post-thumb">
				<a href="#"><img src="{$image_baseurl|escape:'htmlall':'UTF-8'}thumb_{$post.image|escape:'htmlall':'UTF-8'}" alt="{$post.title|escape:'htmlall':'UTF-8'}" class="img-responsive" /></a>			 		
			</div>
			{/if}	
			...
			<ul class="post-meta">
				...
				<li class="post-category"><span>{l s='Category' mod='jmsblogwidget'} :</span> <a href="{jmsblog::getPageLink('jmsblog-category', $catparams)|escape:'htmlall':'UTF-8'}">{$post.category_name|escape:'htmlall':'UTF-8'}</a></li>
				{/if}					
				<li class="post-created"><span>{l s='Created' mod='jmsblogwidget'} :</span> {$post.created|escape:'htmlall':'UTF-8'|date_format:"%B %e, %Y"}</li>
				{if $widget_setting.JBW_SHOW_VIEWS}
				<li class="post-views"><span>{l s='Views' mod='jmsblogwidget'} :</span> {$post.views|escape:'htmlall':'UTF-8'}</li>
				{/if}
				{if $widget_setting.JBW_SHOW_COMMENT}
				<li class="post-comments"><span>{l s='Comments' mod='jmsblogwidget'} :</span> {$post.comment_count|escape:'htmlall':'UTF-8'}</li>
				{/if}	
			</ul>
			...
			<a class="blog-readmore">{l s='read more' mod='jmsblogwidget'} ...</a>	
			{/if}
		</div>	
	{/foreach}
	</div>
