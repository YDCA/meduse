...
<div class="blog-carousel">	
	{foreach from=$posts item=post}				
		...
		<div class="blog-item">
			...
			<div class="post-thumb">
				<a href="#"><img src="{$image_url|escape:'html':'UTF-8'}thumb_{$post.image|escape:'html':'UTF-8'}" alt="{$post.title|escape:'htmlall':'UTF-8'}" class="img-responsive" /></a>			 		
			</div>
			{/if}	
			...
			<ul class="post-meta">
				...
				{/if}
				{if $show_ncomments == '1'}		
				<li class="post-comments"><span>{l s='Comments' mod='jmspagebuilder'} :</span> {$post.comment_count|escape:'html':'UTF-8'}</li>
				{/if}	
			</ul>
			{if $show_introtext == '1'}	
			...
			{/if}
			{if $show_readmore == '1'}	
				<a class="post-readmore">{l s='read more' mod='jmspagebuilder'} ...</a>	
			{/if}
		</div>	
	{/foreach}	
</div>	
