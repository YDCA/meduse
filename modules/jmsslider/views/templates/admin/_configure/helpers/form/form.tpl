...
{block name="field"}
	{if $input.type == 'img'}	
		<div class="col-sm-9">				
			...
			<div class="dummyfile input-group">
				...
				<input id="{$input.name|escape:'htmlall':'UTF-8'}-name" type="text" name="filename" readonly />
			...
			<p class="help-block">{$input.pdesc|escape:'htmlall':'UTF-8'}</p>			
			<img src="{$image_baseurl|escape:'htmlall':'UTF-8'}{$fields[0]['form']['old_image']|escape:'htmlall':'UTF-8'}" class="img-thumbnail" />
			<input type="hidden" name="old_image" value="{$fields[0]['form']['old_image']|escape:'htmlall':'UTF-8'}" />
			{/if}			
			</div>
		</div>
		<script>
	...
	{elseif $input.type == 'layer_img'}
		{$input.file_list|escape:'htmlall':'UTF-8'}
