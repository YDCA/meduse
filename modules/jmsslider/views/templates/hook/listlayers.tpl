...
<form id="layer_form" action="{$link->getAdminLink('AdminModules')|escape:'htmlall':'UTF-8'}&configure=jmsslider&layer=1" method="post">
<script>
$(function() {
                var $myLayers = $("#layers");
                $myLayers.sortable({
...
<div class="panel area-display">
	...
	{foreach from=$all_slides item=eve_slide}
		<a {if $eve_slide.id_slide==$slides.id_slide}class="btn-info"{else}title="{l s='Click here go to ' mod='jmsslider'}{$eve_slide.title|escape:'htmlall':'UTF-8'}"{/if} href="{$link->getAdminLink('AdminModules')|escape:'htmlall':'UTF-8'}&configure=jmsslider&layers=1&id_slide={$eve_slide.id_slide|escape:'htmlall':'UTF-8'}">{$eve_slide.title|escape:'htmlall':'UTF-8'}</a>&nbsp;&nbsp;
	{/foreach}</div>
	</h3>
	<div class="panel-body ">
		<div class="wrap-slider">
			<div  class="slider" style="width:{$configs.JMS_SLIDER_WIDTH|escape:'htmlall':'UTF-8'}px;height:{$configs.JMS_SLIDER_HEIGHT|escape:'htmlall':'UTF-8'}px">
				<div id="frame_layer" class="slide" style="{if $slides.bg_type==1}background-image:url({$root_url|escape:'htmlall':'UTF-8'}modules/jmsslider/views/img/slides/{$slides.bg_image|escape:'htmlall':'UTF-8'});{else}background-color:{$slides.bg_color|escape:'htmlall':'UTF-8'}{/if};background-size:100% 100%;position:relative;width:100%;height:100%">
					{foreach from=$layers item=layer}
						...
						{if $layer.data_type=="text"}
							<span>{$layer.data_html|escape:'htmlall':'UTF-8'}</span>
							...
							{if $layers|@count gt 0}				
								...
								<div id="layers_{$layer.id_layer|escape:'htmlall':'UTF-8'}" class="panel layer layers-{$layer.id_layer|escape:'htmlall':'UTF-8'}">
									<div class="row">
										...
										<div class="col-md-6">							
											{$layer.data_title|escape:'htmlall':'UTF-8'}
										</div>
										<div class="col-md-5">							
