		...
		{foreach from=$slides item=slide}
			<div class="slide {$slide.class_suffix|escape:'htmlall':'UTF-8'}" style="background:{$slide.bg_color|escape:'htmlall':'UTF-8'} url({$root_url|escape:'htmlall':'UTF-8'}modules/jmsslider/views/img/slides/{$slide.bg_image|escape:'htmlall':'UTF-8'}) no-repeat left top;background-size:cover;" {if $slide.slide_link}onclick="document.location='{$slide.slide_link|escape:'htmlall':'UTF-8'}';"{/if}>
				{foreach from=$slide.layers item=layer}
					...
					data-fontsize = "{$layer.data_font_size|escape:'htmlall':'UTF-8'}"
					{if $layer.data_line_height}
					data-lineheight = "{$layer.data_line_height|escape:'htmlall':'UTF-8'}px"
					{/if}
					...
					style="font-size: {$layer.data_font_size|escape:'htmlall':'UTF-8'}px; font-style:{$layer.data_style|escape:'htmlall':'UTF-8'}; color: {$layer.data_color|escape:'htmlall':'UTF-8'}; line-height:{if $layer.data_line_height}{$layer.data_line_height|escape:'htmlall':'UTF-8'}px{/if};"					
					>{$layer.data_html|escape:'htmlall':'UTF-8'}
					</div>
...
<script type="text/javascript">
	$(window).load(function(){
		$('.slider').fractionSlider({	
			'slideTransition' : "{$configs.JMS_SLIDER_TRANS|escape:'htmlall':'UTF-8'}",
			'slideEndAnimation' : {if $configs.JMS_SLIDER_END_ANIMATE}true{else}false{/if},
			'transitionIn' : "{$configs.JMS_SLIDER_TRANS_IN|escape:'htmlall':'UTF-8'}", // default in - transition
			'transitionOut' : "{$configs.JMS_SLIDER_TRANS_OUT|escape:'htmlall':'UTF-8'}", // default out - transition
			'fullWidth' : {if $configs.JMS_SLIDER_FULL_WIDTH}true{else}false{/if}, // transition over the full width of the window
			'delay' : {$configs.JMS_SLIDER_DELAY|escape:'htmlall':'UTF-8'}, // default delay for elements
			'timeout' : {$configs.JMS_SLIDER_DURATION|escape:'htmlall':'UTF-8'}, // default timeout before switching slides
			'speedIn' : {$configs.JMS_SLIDER_SPEED_IN|escape:'htmlall':'UTF-8'},
			'speedOut' : {$configs.JMS_SLIDER_SPEED_OUT|escape:'htmlall':'UTF-8'}, // default in - transition speed
			'easeIn' : "{$configs.JMS_SLIDER_EASE_IN|escape:'htmlall':'UTF-8'}", // default easing in
			'easeOut' : "{$configs.JMS_SLIDER_EASE_OUT|escape:'htmlall':'UTF-8'}", // default easing out
			...
			'backgroundEase' : "{$configs.JMS_SLIDER_BG_EASE|escape:'htmlall':'UTF-8'}",
			'responsive' : {if $configs.JMS_SLIDER_RESPONSIVE}true{else}false{/if}, // responsive slider (see below for some implementation tipps)
			'dimensions' : "{$configs.JMS_SLIDER_WIDTH|escape:'htmlall':'UTF-8'},{$configs.JMS_SLIDER_HEIGHT|escape:'htmlall':'UTF-8'}",
		});
	});
</script>