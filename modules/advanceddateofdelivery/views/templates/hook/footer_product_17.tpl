...
<section class="page-product-box" id="advanceddateofdelivery_footer_product" {if $adod_product_display == 'PRODUCT_FANCYBOX'}style="display:none"{/if}>
	<h3 class="h5 text-uppercase">{l s='Delivery date' mod='advanceddateofdelivery'}</h3>
					...
					<tr class="{cycle values="odd,even"}" id="carrierItem_{$carrier.id_carrier|escape:'htmlall':'UTF-8'}">
						...
						<td>
							{if $carrier.price}
								<span class="price">
									{if $priceDisplay}
										{Tools::displayPrice($carrier.price_tax_exc)} {if $configuration.display_taxes_label} {l s='(tax excl.)' mod='advanceddateofdelivery'}{/if}
									{else}
										{Tools::displayPrice($carrier.price)} {if $configuration.display_taxes_label} {l s='(tax incl.)' mod='advanceddateofdelivery'}{/if}
									{/if}
								</span>
							{else}
