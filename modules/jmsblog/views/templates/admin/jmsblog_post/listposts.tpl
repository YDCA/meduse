				...
				<tr id="posts_{$post.post_id|escape:'htmlall':'UTF-8'}" class="{if $i%2 == 1}odd{/if}">					
					...
					<td>
						<div class="btn-group-action pull-right">
							<a class="btn {if $post.active}btn-success{else}btn-danger{/if}"	href="{$link->getAdminLink('AdminJmsblogPost')|escape:'htmlall':'UTF-8'}&configure=jmsblog&status_id_post={$post.post_id|escape:'htmlall':'UTF-8'}&changePostStatus" title="{if $post.active}Enabled{else}Disabled{/if}">
							...
							<a class="btn btn-default"
									href="{$link->getAdminLink('AdminJmsblogPost')|escape:'htmlall':'UTF-8'}&configure=jmsblog&delete_id_post={$post.post_id|escape:'htmlall':'UTF-8'}" onclick="return confirm('Are you sure you want to delete this item?');">
