...
<h1>{l s='Customer service' mod='recaptcha'} - {if isset($customerThread) && $customerThread}{l s='Your reply' mod='recaptcha'}{else}{l s='Contact us' mod='recaptcha'}{/if}</h1>

{if isset($confirmation)}
	<p>{l s='Your message has been successfully sent to our team.' mod='recaptcha'}</p>
	<ul class="footer_links">
		<li><a href="{$base_dir}"><img class="icon" alt="" src="{$img_dir}icon/home.gif"/></a><a href="{$base_dir}">{l s='Home' mod='recaptcha'}</a></li>
	</ul>
{elseif isset($alreadySent)}
	<p>{l s='Your message has already been sent.' mod='recaptcha'}</p>
	<ul class="footer_links">
		<li><a href="{$base_dir}"><img class="icon" alt="" src="{$img_dir}icon/home.gif"/></a><a href="{$base_dir}">{l s='Home' mod='recaptcha'}</a></li>
	</ul>
{else}
	<p class="bold">{l s='For questions about an order or for more information about our products' mod='recaptcha'}.</p>
	{include file="$tpl_dir./errors.tpl"}
	<form action="{$request_uri|escape:'htmlall':'UTF-8'}" method="post" class="std" enctype="multipart/form-data">
		<fieldset>
			<h3>{l s='send a message' mod='recaptcha'}</h3>
			<p class="select">
		...
		{if !$PS_CATALOG_MODE}
			...
			<p class="text select">
				<label for="id_order">{l s='Order reference' mod='recaptcha'}</label>
			...
			<p class="text select">
			<label for="id_product">{l s='Product' mod='recaptcha'}</label>
		...
		<p class="submit">
			<input type="submit" name="submitMessage" id="submitMessage" value="{l s='Send' mod='recaptcha'}" class="button_large" />
		</p>
	</fieldset>
</form>
{/if}