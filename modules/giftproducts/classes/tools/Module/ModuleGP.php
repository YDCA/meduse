...
class ModuleGP extends Module
{
    ...
    {
        $this->name = ToolsModuleGP::getModNameForPath(__FILE__);
    ...
    public function installClasses()
    {
        foreach ($this->classes as $class) {
            HelperDbGP::loadClass($class)->installDb();
        }
        return true;
    }
    ...
    {
        foreach ($this->classes as $class) {
            HelperDbGP::loadClass($class)->uninstallDb();
        }
        return true;
    }
    ...
    public function formatTabs($tabs)
    {
        if (version_compare(_PS_VERSION_, '1.7.1.0', '>=')) {
            foreach ($tabs as &$tab) {
                ...
                $languages = ToolsModuleGP::getLanguages(false);
                $name = array();
    ...
    public function installTabs()
    {
        if (version_compare(_PS_VERSION_, '1.7.1.0', '<')) {
            foreach ($this->tabs as $tab) {
                ToolsModuleGP::createTab($this->name, $tab['tab'], $tab['parent'], $tab['name']);
            }
        }
        return true;
    }
    ...
    {
        DocumentationGP::assignDocumentation();
        $return_back_link = '#';
        if (count($this->tabs)) {
            $return_back_link = $this->context->link->getAdminLink(
                isset($this->tabs[0]['class_name']) ? $this->tabs[0]['class_name'] : $this->tabs[0]['tab']
            );
        }

        $this->context->smarty->assign('return_back_link', $return_back_link);
        return ToolsModuleGP::fetchTemplate('admin/documentation.tpl');
    }
    ...
    public function getContent()
    {
        if (!$this->documentation) {
            ...
            SmartyGP::registerSmartyFunctions();
            $this->context->smarty->assign(array(
                'content_tab' => $this->getContentTab(),
                'documentation' => $this->getDocumentation()
            ));
            return ToolsModuleGP::fetchTemplate('admin/content.tpl');
        }
    }
    ...
    public function importFixtures()
    {
        ...
        $fixture_class = FixtureGP::getInstance($this->name);
