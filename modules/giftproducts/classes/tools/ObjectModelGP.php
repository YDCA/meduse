...
class ObjectModelGP extends ObjectModel
{
    /**
     * @var int
     */
    ...
    public function getImage()
    {
        if (static::$lang_image) {
            if (!is_null($this->id_lang)) {
                ...
                $images = array();
                foreach (ToolsModuleGP::getLanguages(!defined('_PS_ADMIN_DIR_')) as $l) {
    ...
    public function uploadImage($tmp_name, $id_lang = null)
    {
        ...
        if ($tmp_name) {
            ...
            if (ToolsModuleGP::checkImage($tmp_name)) {
                $width = null;
                $height = null;
    ...
    {
        return _PS_MODULE_DIR_.ToolsModuleGP::getModNameForPath(__FILE__).'/views/img/'
    ...
    {
        return _MODULE_DIR_.ToolsModuleGP::getModNameForPath(__FILE__).'/views/img/'
            .Tools::strtolower($this->getClassName()).'/'
            .(int)$this->id.(!is_null($id_lang) ? '_'.$id_lang : '').'.jpg';
    }
