...
class GiftCategory extends ObjectModel
{
    ...
    public static function getGiftProductsByCategories($id_lang, $ids_categories, $id_cart)
    {
        if (!is_array($ids_categories) || !count($ids_categories)) {
            return array();
        }
        ...
        $products = Db::getInstance()->executeS(
            ...
            SUM((
                ...
                LEFT JOIN '._DB_PREFIX_.'category_product cat_p
                    ON cat_p.`id_product` = cp.`id_product`
                WHERE cat_p.`id_category` = gc.`id_category` AND cp.`id_cart` = '.(int)$id_cart.'
            )) as quantity
            FROM '._DB_PREFIX_.'gift_category gc
            LEFT JOIN '._DB_PREFIX_.'product p ON p.`id_product` = gc.`id_gift`
