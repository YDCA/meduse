...
jQuery(function ($) {
    "use strict";
	var numbercat = {$jmsitems|escape:'htmlall':'UTF-8'};
			...
			<div class="item">
				...
				{if $show_image}
					<div class="product_img" >
						<a href="{$product.link|escape:'htmlall':'UTF-8'}" class="preview-image product_img_link">
							<img class="img-responsive" src="{$image_url|escape:'htmlall':'UTF-8'}resized_{$product.image|escape:'htmlall':'UTF-8'}" />
						</a>
					</div>
				{/if}
					<div class="product_author" {if $show_office == 0} class="no-office"{/if}>
						{$product.author|escape:'htmlall':'UTF-8'}{if $show_office} - {$product.office|escape:'htmlall':'UTF-8'} {/if}
					</div>
					<div class="product_date" {if $show_date == 0} class="no-date"{/if}>
						{if $show_date}
							{$product.posttime|escape:'htmlall':'UTF-8'}
						{/if}
					</div>
