...
require_once(dirname(__FILE__).'/libraries/Chronolog.php');
...
website to download a comptible version.';
    ...
    public static function createCarrier($code)
    {
        ...
        $carrier->active = true;
        $carrier->deleted = 0;
        $delays = self::$carriers_definitions[$code]['delay'];
        ...
        foreach (Language::getLanguages(true) as $language) {
            if (array_key_exists($language['iso_code'], self::$carriers_definitions[$code]['delay'])) {
                $carrier->delay[$language['id_lang']] =substr(self::$carriers_definitions[$code]['delay'][$language['iso_code']],0,128);
            } else {
                $carrier->delay[$language['id_lang']] = substr(self::$carriers_definitions[$code]['delay']['fr'],0,128);
            }
        }
    ...
    public static function gettingReadyForSaturday($carrier = null)
    {
        if (Configuration::get('CHRONOPOST_SATURDAY_ACTIVE') != 'yes') {
            return false;
        }

        if ($carrier != null && $carrier->id_reference == Configuration::get('CHRONOPOST_DIMANCHEBAL_ID')) {
            return false;
        }
    ...
    private function carrierForm($code)
    {
        ...
        $this->context->smarty->assign(
            array(
                ...
                'selected' => $selected,
                'code' => $code,
                'code_label' => isset($labels_map[$code]) ? $labels_map[$code] : $code
            )
        );
