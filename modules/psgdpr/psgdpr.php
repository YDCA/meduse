...
class Psgdpr extends Module
{
    private $settings_data_consent = array(
        ...
        'switchContactForm' => 'psgdpr_contact_form_switch',
        'contactForm' => 'psgdpr_contact_form',
    );
    ...
    public function install()
    {
        ...
        foreach ($this->settings_data_consent as $value) {
            if ($value === 'psgdpr_creation_form' || $value === 'psgdpr_contact_form') {
        ...
        if (parent::install() &&
            ...
            if ($this->ps_version) { // if 1.7
                if ($this->registerHook($hook17)) {
                    return true;
                }
            } else { // if 1.6
                if ($this->registerHook($hook16)) {
                    return true;
                }
            }
    ...
    public function loadAsset()
    {
        ...
        $jss = array(
            ...
            _PS_ROOT_DIR_.'js/tiny_mce/tinymce.min.js',
    ...
    public function getContent()
    {
        ...
        foreach ($this->settings_data_consent as $index => $value) {
            if ($value === 'psgdpr_creation_form' || $value === 'psgdpr_customer_form' || $value === 'psgdpr_contact_form') {
                foreach ($languages as $lang) {
    ...
    public function hookDisplayGDPRConsent($params)
    {
        ...
        if (isset($params['moduleName']) && $params['moduleName'] === 'contactform') {
            return $this->displayContactFormCheckbox();
        }
    ...
    public function displayContactFormCheckbox()
    {
        $id_lang = Context::getContext()->language->id;
        $id_customer = Context::getContext()->customer->id;
        $id_guest = Context::getContext()->cart->id_guest;
        ...
        $frontController = Context::getContext()->link->getModuleLink($this->name, 'FrontAjaxGdpr', array(), true);
        $active = Configuration::get('PSGDPR_CONTACT_FORM_SWITCH');

        if (!$active) {
            return;
        }

        $label = Configuration::get('PSGDPR_CONTACT_FORM', $id_lang);
        $this->context->smarty->assign(array(
            'ps_version' => $this->ps_version,
            'psgdpr_consent_message' => $label,
            'psgdpr_id_module' => 'contactForm',
            'psgdpr_id_customer' => $id_customer,
            'psgdpr_customer_token' => sha1(Context::getContext()->customer->secure_key),
            'psgdpr_guest_token' => sha1('psgdpr'.$id_guest.$_SERVER['REMOTE_ADDR'].date('Y-m-d')),
            'psgdpr_id_guest' => $id_guest,
