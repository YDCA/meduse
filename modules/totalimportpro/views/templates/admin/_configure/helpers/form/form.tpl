					...
					<label class="control-label col-lg-3">{$field.label|escape:'htmlall':'UTF-8'}</label>
						...
						<input type="checkbox"
							   ...
							   class="{if isset($field.class)}{$field.class|escape:'htmlall':'UTF-8'}{/if}"
								{if isset($fields_value[$field.id]) AND $fields_value[$field.id] === 'on'}checked="checked"{/if} />
								{if isset($field.desc)}
									<p class="help-block">{$field.desc|escape:'htmlall':'UTF-8'}</p>
								{/if}
						</div>
						...
						<ul>
							...
							{foreach $input.shop_groups as $group}
								...
								<label class="check-text">
									<input type="checkbox" onclick="return toggleChecked(this.checked, {$group_num|escape:'htmlall':'UTF-8'});" name="shop_group[]" value="{$group.id|escape:'htmlall':'UTF-8'}"
											 {assign var=in_list value="{$group.id|in_array:$fields_value['shop_group']|escape:'htmlall':'UTF-8'}"}
											 {if $in_list}checked="checked"{/if}
									/>
									...
									{foreach $group.shops as $shop}
										...
										<label class="check-text">
											...
											{assign var=in_list value="{$shop.id_shop|in_array:$fields_value['id_shop_list']|escape:'htmlall':'UTF-8'}"}
											{if $in_list}checked="checked"{/if}
											/>
	...
	{elseif $field.step == 'step3'}
		{block name="field"}
			{if $input.type == 'feed_sample'}
				...
				<div class="stepInfo row">{l s='To reset any changes you make to your data, you must load your feed again in step 1.' mod='totalimportpro'}</div>
					<!-- Sample Fields -->
				<div id="sampleFields" class="row">
					<p class="h3">{l s='Feed Sample' mod='totalimportpro'}</p>
					<span>{$input.num_rows|escape:'htmlall':'UTF-8'} rows loaded from feed</span>
									...
									{if in_array($input_name, $value)}
										{if !is_array($pretty_name)}
										 ...
										 {elseif $pretty_name[1] == 'combo'}
											...
											-->
											{if !empty($fields_value['field_names'][$pretty_name[0][0][1]])}
												{if count(array_filter(reset($fields_value['field_names'][$pretty_name[0][0][1]]))) > 1}
													{assign var=field_count value=count(array_filter(reset($fields_value['field_names'][$pretty_name[0][0][1]])))}
												{else}
											...
											{assign var=valid_rows value=0}
												...
												<!-- Multi downwards Field -->
													{if isset($fields_value['field_names'][$input_name][$lang['id_lang']]) AND (count($fields_value['field_names'][$input_name][$lang['id_lang']]) > 1)}
														{assign var=valid_rows value=(count($fields_value['field_names'][$input_name][$lang['id_lang']]) - 1)}
													{/if}

													{for $i=0 to $valid_rows}
		...
		{block name="field"}
			...
			{elseif $input.type == 'range'}
				...
				<div id="settings_profile" title="Settings Profile" style="display:none;">
					...
					<p>{l s='Or pick a saved profile to overwrite:' mod='totalimportpro'}</p>
					<select id="profile-select">
						<option value="">[New Profile]</option>
					{foreach $input.profiles as $profile}
						<option value="{$profile|escape:'htmlall':'UTF-8'}">{$profile|escape:'htmlall':'UTF-8'}</option>
					{/foreach}
					</select>
				</div>
			{else}
