...
class TotalImportPRO extends Module
{
	...
	public function getContent()
	{
		...
		$this->_html .= '
		<script>
			var TotalImportPRO = {}; // Global TIP Variables
		</script>';

		$this->context->controller->addJs(_MODULE_DIR_.'/totalimportpro/views/js/totalimportpro.js');
		$this->context->controller->addJs(_MODULE_DIR_.'/totalimportpro/views/js/selectize/selectize.js');

		$this->context->controller->addCSS(_MODULE_DIR_.'/totalimportpro/views/css/selectize.css');
		$this->context->controller->addCSS(_MODULE_DIR_.'/totalimportpro/views/css/selectize_bootstrap3.css');
		$this->context->controller->addCSS(_MODULE_DIR_.'/totalimportpro/views/css/totalimportpro.css');

		if (!$this->feedFileWritable())
			$this->errors[] = 'Warning: Your feed.txt file needs to have read/write permissions set';
	...
	{
		$max = ini_get('post_max_size') | ini_get('upload_max_filesize');
		return $max;
	}
	...
	public function cron()
	{
		if (defined('CLI_INITIATED'))
		{
			...
			if (defined('EMPLOYEE_EMAIL') && Employee::employeeExists(EMPLOYEE_EMAIL))
			{
				$_employee = new Employee();
				$employee = $_employee->getByEmail(EMPLOYEE_EMAIL);
			...
			else
			{
				...
				if (isset($employees[0]))
					$this->context->employee = $employees[0];
			}
			...
			print_r(sprintf($this->l('Success: Added %s products, Updated %s products, Affected %s not in file, Added %s combinations, Updated %s combinations'),
				$this->total_items_added, $this->total_items_updated, $affected_products, $this->total_combinations_added, $this->total_combinations_updated));
		}
	}
		...
		while (($raw_prod = $this->getNextProduct($product_num)))
		{
			...
			foreach ($field_groups as $field_group)
			{
				...
				if (isset($settings[$field_group]['visibility']) && isset($raw_prod[$settings[$field_group]['visibility']]))
				{
					...
					$visibility_field = preg_match('/^everywhere|catalog|search|none|^$/i', $field_val) ? Tools::strtolower($field_val) : 'everywhere';
			...
			if ($settings['new_items'] != 'skip' && !$update_id)
			{
				//New Product
	...
	*/
	public function importProduct(&$raw_prod, &$settings, &$update_id = 0, &$combination_id = 0)
	{
		...
		else
		{
			...
			foreach ($this->global_data as $field => $default_value)
				$product->{$field} = ($default_value == 'on') ? 1 : $default_value; //on is the .val() of a checkbox
		}
		...
		foreach ($this->languages as $lang)
		{
			if (!empty($raw_prod[$settings['field_names']['name'][$lang['id_lang']]]))
			{
				...
				if (isset($settings['field_names']['link_rewrite'][$lang['id_lang']]) && !empty($raw_prod[$settings['field_names']['link_rewrite'][$lang['id_lang']]]))
				{
			...
			$keywords = array();
			if (isset($settings['field_names']['meta_keywords'][$lang['id_lang']]) && !empty($raw_prod[$settings['field_names']['meta_keywords'][$lang['id_lang']][0]]))
			{
			...
			if (isset($settings['field_names']['meta_description'][$lang['id_lang']]) && !empty($raw_prod[$settings['field_names']['meta_description'][$lang['id_lang']]]))
			{
		...
		foreach ($this->desc_data as $field => $value)
		{
			if (array_key_exists($field, $prod_vars))
			{
				...
				{
					//don't add default value for update
					if (isset($settings['field_names'][$field][$id_lang]) && !empty($raw_prod[$settings['field_names'][$field][$id_lang]]))
		...
		foreach ($global_multi as $global)
		{
			if (array_key_exists($global, $prod_vars))
			{
				foreach ($this->languages as $lang)
				{
					if (isset($settings['field_names'][$global][$lang['id_lang']]) && isset($raw_prod[$settings['field_names'][$global][$lang['id_lang']]]))
		...
		foreach ($this->languages as $lang)
		{
			$first_cat_column_name = isset($settings['field_names']['category'][$lang['id_lang']][0][0]) ? $settings['field_names']['category'][$lang['id_lang']][0][0] : '';
		...
		if ($settings['validate_product'] == 0)
		{
			...
			$product = $this->validateProductValues($product);
		...
		if ($valid)
		{
			if ($update_id)
			{
				...
				if ($action)
				{
					...
					Search::indexation(false, $update_id);
				}
			}
			else
			{
		...
		if ($action)
		{
			...
			foreach ($this->languages as $lang)
			{
				if (isset($settings['field_names']['tag'][$lang['id_lang']]) && !empty($raw_prod[$settings['field_names']['tag'][$lang['id_lang']][0]]))
				{
					$tag_mapped = true;
					break;
				}
			...
			//Specific Prices
			if (isset($settings['field_names']['specific_price'][0]) && isset($raw_prod[$settings['field_names']['specific_price'][0]]))
			...
			if (isset($settings['field_names']['carriers'][0]) && isset($raw_prod[$settings['field_names']['carriers'][0]]))
			...
			if ($update_id && $settings['delete_images'] && !$isCombinationUpdate)
				$this->deleteImages($product->id);
			if (isset($settings['field_names']['image'][0]) && isset($raw_prod[$settings['field_names']['image'][0]]) || !empty($raw_prod[$settings['field_names']['cover_image']]))
			{

				$field_images = array_merge(array($settings['field_names']['cover_image']), $settings['field_names']['image']);
				$field_images = array_unique($field_images);
				$i = 1;
			...
			if (isset($settings['field_names']['custom_file'][$this->languages[0]['id_lang']]) && isset($raw_prod[$settings['field_names']['custom_file'][$this->languages[0]['id_lang']][0]])
				|| isset($settings['field_names']['custom_text'][$this->languages[0]['id_lang']]) && isset($raw_prod[$settings['field_names']['custom_text'][$this->languages[0]['id_lang']][0]]))
			{
			...
			//Attachments
			if (isset($settings['field_names']['attachment'][$this->languages[0]['id_lang']]) && !empty($raw_prod[$settings['field_names']['attachment'][$this->languages[0]['id_lang']][0]]))
			...
			// 	// $associations = array('accessories');
			// 	foreach ($associations as $association)
			// 	{
			// 		if (isset($settings['field_names'][$association][0]) && isset($raw_prod[$settings['field_names'][$association][0]]))
			// 		{
	 ...
	 * @return Product
	 */
	public function validateProductValues($product)
	{
			...
			if (isset($data['validate']))
			{
				if ($lang)
				{
					foreach ($this->languages as $language)
					{
						...
						if (isset($product->{$field}[$language['id_lang']]))
							$validated_value = $this->autoValidateLangField($field, $data, $product->{$field}[$language['id_lang']], $language['id_lang']);
						$product->{$field}[$language['id_lang']] = ($validated_value) ? $validated_value : '';
					}
				}
				else
					$product->$field = $this->autoValidateField($field, $data, $product);
			}
		}
	...
	/* Auto-validate a lang field
	 *
	 * @param $field, string, Name of field being validated
	 * @param $fieldData, array, Array of field data
	 * @param $productVal, string, Product value to be validated
	 * @parma $lang, int, Language id
	 *
	 * @return string
	 */
	public function autoValidateLangField($field, $fieldData, $productVal, $lang)
	{
		$default_val = $this->defaultValueForType($fieldData['type']);
		$validated_value = $default_val; //preset val to default
		if (isset($fieldData['required']))
			$default_val = $this->desc_data[$field][$lang];
		if (isset($productVal))
		{
			$new_value = $this->autoValidate($productVal, $fieldData);
			if ($new_value != false)
				$validated_value = ($new_value == 'empty') ? $default_val : $new_value;
		}

		return $validated_value;
	}

	/* Auto-validate a field
	 *
	 * @param $field, string, Name of field being validated
	 * @param $fieldData, array, Array of field data
	 * @param $productVal, string, Product value to be validated
	 ...
	 * @return string
	 */
	public function autoValidateField($field, $fieldData, $product)
	{
		$default_val = $this->defaultValueForType($fieldData['type']);
		$validated_value = $default_val; //preset val to default
		if (isset($fieldData['required']))
			$default_val = $this->prod_data[$field];
		if (isset($product->{$field}))
		{
			$new_value = $this->autoValidate($product->{$field}, $fieldData);
			if ($new_value != false)
				$validated_value = ($new_value == 'empty') ? $default_val : $new_value;
		}
	...
	/**
	 * Get a valid default value based on the field type
	 *
	 * @param $type int @see ObjectModel type definition
	 *
	 * @return mixed
	 */
	public function defaultValueForType($type)
	{
		$TYPE_INT     = 1;
		$TYPE_BOOL    = 2;
		$TYPE_STRING  = 3;
	...
	public function importCSV($filename, $delimiter, $csv_options)
	{
		...
		do {
			...
			while (($row = fgetcsv($fh, 0, $delimiter)) !== false)
			{
				...
				{
					if ($this->areExtraColumnsEmpty($row, $num_cols))
					{
						$row = $this->removeEmptyRows($row, $num_cols);
						break;
					}
					else
	...
	public function areExtraColumnsEmpty($row, $columnCount)
	{
		$limit = count($row);
		for ($i = $columnCount; $i != $limit; $i++)
		{
			if (Tools::strlen(trim($row[$i])) !== 0)
				return false;
		}
		return true;
	}

	public function removeEmptyRows($row, $columnCount)
	{
	...
	public function getOperations()
	{
		$operations = array(
			...
			'dividePrice' => array(
				'name' => $this->l('Adjust Price (Divide)'),
				'function' => 'divide',
				'inputs'=>array(
					array('type'=>'field', 'prepend'=>$this->l('Divide')),
					array('type'=>'text', 'prepend'=>$this->l('by'))
				),
				'label' => $this->l('Most Popular'),
			),
	...
	private function deleteExistingProds($delete_diff, $update_field)
	{
		...
		$this->logger->logInfo(sprintf('There are %s products in store but not in feed', count($existing_prods)));
	...
	public function createDeleteDiffTable($update_field, $settings)
	{
		...
		$sql = 'CREATE TABLE IF NOT EXISTS '._DB_PREFIX_.'hj_existing_prods (product_id INT(11) AUTO_INCREMENT, product_id_field VARCHAR(64), PRIMARY KEY (product_id))';
		...
		if (!empty($existing))
		{
			...
			for ($prod_num = 1; $prod_num < $num_prods; $prod_num++)
			{
				...
				if ($prod_num % 999 == 0)
				{
					Db::getInstance()->execute($sql);
					$prod_num++;
					if ($prod_num >= $num_prods)
						return;
					$sql = 'INSERT INTO '._DB_PREFIX_.'hj_existing_prods (product_id, product_id_field)';
					$sql .= ' VALUES ('.$existing[$prod_num]['id_product'].', \''.$existing[$prod_num][$update_field].'\')';
				}
			}
			Db::getInstance()->execute($sql);
		}
	...
	private function importEndCleanUp()
	{
		$this->initFileLogger();
		$this->logger->logInfo('Import finished, running cleanup tasks.');
		$this->ajaxProcessRegenCatTree();
		$this->logger->logInfo('Cleanup tasks finished.');
	}
	...
	public function ajaxProcessRegenCatTree()
	{
		...
		$this->logger->logInfo('Regenerated Category Tree.');
	}
	...
	public function ajaxProcessImport()
	{
		...
		$this->logger->logInfo('Updated '.$this->total_items_updated.' and Added '.$this->total_items_added.' Products. '.$this->total_items_invalid.' products invalid.');

		echo Tools::jsonEncode(array('limit' => $limit, 'added' => $this->total_items_added, 'updated' => $this->total_items_updated, 'invalid' => $this->total_items_invalid));
	}
	...
	public function importSettings()
	{
		...
		if (!isset($_FILES['settings_file']))
			$this->errors[] = 'Error: You are trying to import the wrong file';
		else
		{
		...
		{
			if (isset($fileExtension[1]) && $fileExtension[1] !== 'csv')
				$this->errors[] = 'Error Importing Settings: Incorrect CSV format (try exporting again)';
			else
			{
				$l = $file['tmp_name'];
				$csv_body = Tools::file_get_contents($l);

				//Removing the \r line endings that Windows adds on to make the CSV standard
				$csv = str_replace("\r", '', $csv_body);
				$csv = explode("\n", $csv);

				$headers = explode(',', array_shift($csv));

				$settings = array();
				if (count($headers) !== 2 || $headers[0] !== 'setting_name' || $headers[1] !== 'setting_value')
					$this->errors[] = 'Error Importing Settings: Incorrect CSV format (try exporting again)';
				else
				{
	...
	public function feedFileWritable()
	{
		$filename = _PS_MODULE_DIR_.'totalimportpro/feed.txt';
	...
	private function _postProcess()
	{
		...
		if (Tools::isSubmit('submitStep'))
		{
			...
			{
				$adjusts = Tools::getValue('adjust');
				foreach ($adjusts as $key => $value)
				{
					foreach ($value as $key2 => $string)
						$adjusts[$key][$key2] = htmlentities($string);
				}
			...
			elseif (Tools::getValue('submitStep4'))
			{
				...
				$cleanFields = $this->removeEmptyFields($_POST);
				$this->updateConfig('IMPORT_STEP4', serialize($cleanFields));
	...
	protected function displayForm()
	{
		...
		//Add the Cron Paths as an HTML comment
		$php_path = PHP_BINDIR.'/php';
		$cron_index_path = dirname(__FILE__).'/cron_index.php';
		$this->_html .= '<!-- Cron Command: '.$php_path.' '.$cron_index_path.' -->';

		$this->_html .= '
		<script>
			TotalImportPRO.php_path = "'.$php_path.'";
			TotalImportPRO.cron_index_path = "'.$cron_index_path.'";
			TotalImportPRO.ajax_url = "'.$this->context->link->getAdminLink('AdminModules').'&configure='.$this->name.'";
		</script>';
		...
		if (empty($saved_settings))
			$this->_html .= '<tr><td>[No Profiles Saved]</td><td></td></tr>';

		foreach ($saved_settings as $setting)
		{
			$cron_compatible = $this->profileSuitableForCron($setting);

			$cron_button_classes = 'btn btn-primary cron-profile';
			$cron_button_tooltip = 'data-toggle="tooltip" data-placement="top" data-title="Cron Import"';
			if (!$cron_compatible)
			{
				$cron_button_classes .= ' disabled';
				$cron_button_tooltip = '';
			}
			$cron_button_html = '<button '.$cron_button_tooltip.' class="'.$cron_button_classes.'" data-profile="'.$setting.'"><i class="icon-clock-o"></i></button>';
			if (!$cron_compatible)
				$cron_button_html = '<div class="tooltip-wrapper" data-toggle="tooltip" data-placement="top" data-title="File Upload not suitable for Cron Imports. Use URL, FTP, or Filesystem path.">'.$cron_button_html.'</div>';

			$this->_html .= '
				<tr class="profile_row">
					<td>'.$setting.'</td>
					<td class="pull-right">
						<button class="btn btn-success load-profile" data-profile="'.$setting.'" data-toggle="tooltip" data-placement="top" data-title="Load Profile"><i class="icon-upload"></i></button>
						'.$cron_button_html.'
						<button class="btn btn-danger delete-profile" data-profile="'.$setting.'" data-toggle="tooltip" data-placement="top" data-title="Delete Profile"><i class="icon-trash"></i></button>
					</td>
				</tr>';
		}
		...
		$this->_html .= '
				...
				<a href="#" class="button btn btn-success" id="uploadProfile" ><span>Upload Settings</span></a>
		...
		<div id="cron-dialog" title="Cron Import URL" style="display:none;">
			<p>This command has been generated for your Cron Import, schedule a cron task to run this command. Only one Import can be running at a time, please allow enough time for the import to finish before scheduling another.</p>
			<p>For a detailed guide to configuring Cron Imports please search the HostJars Helpdesk.</p>
			<input onClick="this.setSelectionRange(0, this.value.length)" readonly="readonly" class="cron-input" type="text" id="cron-dialog-command" value="">
		</div>
		';
		...
		$this->context->controller->addJS(_MODULE_DIR_.$this->name.'/views/js/tip_home.js');
	...
	/**
	 * Removes empty field values from step 4
	 * @param (array) $post - the $_POST containing an array of field_names
	 * @return (array) $_POST with filtered field_names
	 */
	public function removeEmptyFields($post)
	{
		//fields with more setting (can be more than one)
		$normal_vert_fields = array('pack_products', 'specific_price', 'accessories', 'carriers', 'combination_specific_price', 'image', 'supplier', 'warehouse');
		//fields with more setting, and language
		$lang_vert_fields = array('tag', 'meta_keywords', 'feature', 'custom_file', 'custom_text', 'attachment');

		//ensure field names isset or leave
		if (!isset($post['field_names']))
			return $post;
		$fields = $post['field_names']; // make a tmp copy of field_names

		if (!empty($post['field_names']['attribute']) && !empty($post['field_names']['attribute_value']))
		{
			$new_fields = $this->filterAttributeFields($post['field_names']['attribute'], $post['field_names']['attribute_value']);
			$fields['attribute_value'] = $new_fields['attribute'];
			$fields['attribute'] = $new_fields['attribute_value'];
		}

		// iterate the field names
		foreach ($post['field_names'] as $name => $field)
		{
			if (array_search($name, $normal_vert_fields) !== false)
				$new_field = $this->filterNormalVertField($name, $field);
			else if (array_search($name, $lang_vert_fields) !== false)
				$new_field = $this->filterLanguageVertField($name, $field);
			else if ($name == 'category')
				$new_field = $this->filterCategoryField($field);
			else
			{
				//these dont fall into anything, no need to filter, ... continue
				continue;
			}
			//only gets here is a vert or lang field is detected
			//$new_field is set by the specific functions
			if (isset($new_field))
				$fields[$name] = $new_field; // append the new field to the array with key = name
		}
		$post['field_names'] = $fields; // reset to $_POST
		return $post;
	}

	private function filterAttributeFields($attribute_values, $attributes)
	{
		/*
		 $attributes = Array(
			[1] => Array( // Language Key
				[0] => attr_name //Value
				[1] =>
				[2] => attr_name2
			),
		...
		*/
		...
		$filtered_attribute_values = array();
		$language_keys = array_keys($attributes);

		$attribute_count = count($attributes[$language_keys[0]]);
		for ($attribute_num = 0; $attribute_num < $attribute_count; $attribute_num++)
		{
			$value_found = false;
			foreach ($language_keys as $lang_id)
			{
				if (!empty($attributes[$lang_id][$attribute_num]))
				{
					$value_found = true;
					break;
				}
			...
			if ($value_found)
			{
				foreach ($language_keys as $lang_id)
				{
					if (!isset($filtered_attributes[$lang_id]))
						$filtered_attributes[$lang_id] = array();
					if (!isset($filtered_attribute_values[$lang_id]))
						$filtered_attribute_values[$lang_id] = array();
					$filtered_attributes[$lang_id][] = $attributes[$lang_id][$attribute_num];
					$filtered_attribute_values[$lang_id][] = $attribute_values[$lang_id][$attribute_num];
				}
			}
		}
		return array('attribute' => $filtered_attributes, 'attribute_value' => $filtered_attribute_values);
	}

	private function filterCategoryField($categories)
	{
		/*
		$categories = array(
			'1' => array( //Language Key
				'0' => array( //First Category Tree
					'0' => 'Top Level Cat Name',
					'1' => 'Sub-cat Name',
				)
			)
			'2' => array( //Second Language Key
				'0' => array( //First Category Tree
					'0' => 'Top Level Cat Name FR',
					'1' => 'Sub-cat Name FR',
				)
			)
		)
		*/
		...
		$language_keys = array_keys($categories);
		$new_categories = array();

		$cat_tree_count = count($categories[$language_keys[0]]);
		for ($cat_tree_num = 0; $cat_tree_num < $cat_tree_count; $cat_tree_num++)
		{
			$sub_cat_count = count($categories[$language_keys[0]][$cat_tree_num]);
			for ($sub_cat_num = 0; $sub_cat_num < $sub_cat_count; $sub_cat_num++)
			{
				$value_found = false;
				// For each sub-category mapping, check if at least one language has been set
				foreach ($language_keys as $lang_id)
				{
					if (!empty($categories[$lang_id][$cat_tree_num][$sub_cat_num]))
					{
						$value_found = true;
						break;
					}
				}
				// If we found at least one language mapping set, add to new category array
				if ($value_found)
				{
					foreach ($language_keys as $lang_id)
						$new_categories[$lang_id][$cat_tree_num][$sub_cat_num] = $categories[$lang_id][$cat_tree_num][$sub_cat_num];
				}
			}
		}
	...
	/**
	 * @helper to removeEmptyFields() - removes empty values of normal vert fields
	 * @param (string) $name - the name of the field
	 * @param (array) $field - the field values for normal should be array of strings only
	 * @return (array) the filtered $field - or $field if no filtering is needed
	 */
	public function filterNormalVertField($name, $field)
	{
		if (gettype($field) === 'array' && count($field) > 1)
		{ //is filtering needed
			$new_field = array();
			foreach ($field as $value)
			{ //iterate and add if valid
				if (Tools::strlen($value) > 0)
					$new_field[] = $value;
			}
			if (count($new_field) === 0) //dont leave the array empty
				$new_field[] = '';
			return $new_field;
		}
		return $field; // no filtering needed
	}

	/**
	 * @helper to removeEmptyFields() - removes empty values of language vert fields
	 * @param (string) $name - the name of the field
	 * @param (array) $field - the field values for language should be array of language of array of strings
	 * @return (array) the filtered $field
	 *
	 * example $field input:
	 *    array("lang_1" => array(0 => '', 1 => 'field_header_1'),
	 *					"lang_2" => array(0 => '', 1 => 'field_header_2'))
	 *    languages are the first level
	 */
	public function filterLanguageVertField($name, $field)
	{
		$new_field = array();
		$fields = array();
		//first pass finds the fields that have atleast one language entered
		foreach ($field as $langId => $lang)
		{
			foreach ($lang as $fieldId => $value)
			{
				if (Tools::strlen($value) > 0)
					$fields[$fieldId] = true; //lang/field found, add a flag
			}
		}
		foreach ($field as $langId => $lang)
		{
			if (!isset($new_field[$langId]))
				$new_field[$langId] = array();
			foreach ($lang as $fieldId => $value)
			{
				if ($fields[$fieldId] === true) //flag set
					$new_field[$langId][] = $value; //add to the language
			}
			if (count($new_field[$langId]) === 0)
				$new_field[$langId][] = ''; //dont leave things empty
		}
		return $new_field;
	}
	...
	protected function displayStep1()
	{
		...
		$this->fields_form[0]['form'] = array(
			...
			'input' => array(
				...
				array(
					...
					'name' => 'file_encoding',
					'id' => 'file_encoding',
					'desc' => $this->l('If you are unsure, use UTF-8. If your feed is non-English, please double check this setting. Common encodings are English - UTF-8, European - ISO-8859-1'),
					'options' => array(
		...
		function updateText(el, name)
		{
			...
			if (typeof action != \'undefined\')
			{
				...
				else
				{
					$(".xml, .csv").hide();
					if (action === "xml") {
						if ($("select[name=\'file_encoding\']").val() === \'GBK\' || $("select[name=\'file_encoding\']").val() === \'ASCII\' )
							$("select[name=\'file_encoding\']").val(\'UTF-8\');
						$("option[value=\'GBK\']").hide();
						$("option[value=\'ASCII\']").hide();
					} else {
						$("option[value=\'GBK\']").show();
						$("option[value=\'ASCII\']").show();
					}
				}
				$("#"+action+", ."+action).show();
			}
		...
		function saveSettings()
		{
			...
			data = data.replace(/=on/g, \'=1\').replace(/=off/g, \'=0\');
			var url = \''.$ajax_url.'\';
			$.ajax({
				type: "POST",
				url: url,
		...
		{
			if ($(\'#has_headers\').is(\':checked\')) {$(\'#safe_headers\').closest(\'.form-group\').show();}
			else {$(\'#safe_headers\').closest(\'.form-group\').hide();}
			$(\'#has_headers\').change(function () {
				if ($(this).is(\':checked\'))
					$(\'#safe_headers\').closest(\'.form-group\').show();
				else
					$(\'#safe_headers\').closest(\'.form-group\').hide();
			})
	...
	protected function displayStep3()
	{
		...
		$settings = unserialize(Configuration::get('IMPORT_STEP3'));
		if (isset($settings['adjust']))
		{
			foreach ($settings['adjust'] as $key => $value)
			{
				foreach ($value as $key2 => $string)
					$settings['adjust'][$key][$key2] = html_entity_decode($string);
			}
		}
	...
	protected function displayStep5()
	{
		...
		$this->_html .= "
			...
			$(document).ready(function()
			{
				...
				$('#profile-select').on('change', function() {
					console.log('TEST');
					$('#save_settings_name').val($(this).val());
				});
			 ";
		...
		$saved_profiles = $this->getSavedSettingNames();
		...
		<script type="text/javascript">
			...
			$this->_html .= '.after(\'<div class="progress-wrap" style="display:none"><div id="progressbar"></div><div id="import_info"><span class="progress-label"><i class="icon-refresh icon-spin icon-3x"></i> Importing... </span><span>Added: <span id="prod_added">0</span> Updated: <span id="prod_updated">0</span> Skipped / Invalid: <span id="prod_invalid">0</span></span></div></div>\');
						...
						$( "#progressbar" ).progressbar({
							...
							complete: function()
							{
								$(".progress-label").html(\'<i class="icon-refresh icon-spin icon-3x"></i> Running post-import tasks, please wait.. \');
								showAlert(\'Success: \'+parseInt($(\'#prod_updated\').text())+\' products updated, \'+parseInt($(\'#prod_added\').text())+\' products added, \'+parseInt($(\'#prod_invalid\').text())+\' products skipped/invalid. \');
								$.ajax({
									url: \''.$ajax_url.'\',
									type: \'POST\',
									dataType: \'json\',
			...
			function progress(current, first_run){
				...
				$.ajax({
					...
					success: function(result)
					{
						...
						$("#prod_invalid").text(result[\'invalid\'] + parseInt($("#prod_invalid").text()));
			...
			function makeValidProfileName(name)
			{
			...
			function saveSettings(profile)
			{
				if (typeof profile === "undefined") {
					...
					profile = "Import-"+datestring;
				}
				var profile_name = makeValidProfileName($("#save_settings_name").val());
				$("#save_settings_name").val(profile_name);
				var data = $(\'form.totalimportpro\').serialize();
				data = data.replace(/=on/g, \'=1\').replace(/=off/g, \'=0\');
				var url = "'.$ajax_url.'";
				$.ajax({
			...
			case 'step5':
				...
				if (version_compare(_PS_VERSION_, '1.6', '<'))
				{
					$this->toolbar_btn['back'] = array(
						...
						'title' => $this->l('Back'),
						'desc' => $this->l('Back'),
						'js' => 'javascript:saveSettings();window.location.href=\''.$back.'&step=4\';return false;',
						'imgclass' => 'back',
					);
				...
				$this->fields_form[0]['form']['buttons'] = array(
					array(
						'href' => '',
						'js' => 'saveSettings();javascript:window.location.href = \''.$back.'&step=4\';',
						'title' => $this->l('Back'),
						'desc' => $this->l('Back'),
		...
		if (!isset($this->fields_form[0]['form']['buttons']))
		{
			if (version_compare(_PS_VERSION_, '1.6', '<'))
			{
				...
				$this->toolbar_btn['back'] = array(
					...
					'title' => $this->l('Back'),
					'desc' => $this->l('Back'),
					'js' => 'javascript:saveSettings();window.location.href=\''.$back.'&step='.($stepnum - 1).'\';return false;',
					'imgclass' => 'back',
				);
			...
			else
			{
				$this->fields_form[0]['form']['buttons'] = array(
					...
					array(
						...
						'title' => $this->l('Back'),
						'desc' => $this->l('Back'),
						'js' => 'javascript:saveSettings();window.location.href=\''.$back.'&step='.($stepnum - 1).'\'',
						'class' => 'btn-reset btn-info',
						'icon' => 'process-icon-back'
					),
					array(
	...
	private function associateFields($associations, $settings)
	{
		foreach ($associations as $type => $item)
		{
			...
			if ($type == 'pack_products')
			{
				...
				// 		if ($pack_prod_id !== 0)
				// 		{
				// 			if (isset($packed_products[(int)$prod_id]))
				// 			{
				// 				if (array_search((int)$pack_prod_id, $packed_products[(int)$prod_id]) === false)
				// 				{
				// 					Pack::addItem($prod_id, $pack_prod_id, $qty);
				// 					$packed_products[(int)$prod_id][] = (int)$pack_prod_id;
				// 				}
				// 			}
				// 			else
				// 			{
				// 				Pack::addItem($prod_id, $pack_prod_id, $qty);
				// 				$packed_products[(int)$prod_id] = array((int)$pack_prod_id);
				// 			}
				//
				// 			if (isset($packed_products[(int)$pack_prod_id]))
				// 			{
				// 				if (array_search((int)$prod_id, $packed_products[(int)$pack_prod_id]) === false)
				// 				{
				// 					Pack::addItem($pack_prod_id, $prod_id, $qty);
				// 					$packed_products[(int)$pack_prod_id][] = (int)$prod_id;
				// 				}
				// 			}
				// 			else
				// 			{
				// 				Pack::addItem($pack_prod_id, $prod_id, $qty);
				// 				$packed_products[(int)$pack_prod_id] = array((int)$prod_id);
				// 			}
				// 		}
				// 	}
				// }
			...
			if (!isset($attachment_id))
			{
				...
				foreach ($field as $lang => $name)
				{
					...
					if ($attach_details)
						...
						$this->logger->logInfo('This file doesn\'nt exist: '.$attachment->name[$lang].' Please make sure your file for Attaching is in the correct directory. It should be in '.realpath(_PS_DOWNLOAD_DIR_));
				}
				if ($attach_details)
				{
				...
				else
				{
					...
					if ($field_error === true && $lang_field_error === true)
					{
						if ($carr->add())
						{
							...
							$carrier = $this->getCarrierByName($carr->name);
							if (isset($carrier[0]))
							{
								if (isset($carrier[0]['id_reference']))
									$carriers[] = $carrier[0]['id_reference'];
							}
						}
					}
				}
	...
	private function createNewCategory($cat_name, $lang, $parent, $level)
	{
		...
		if ($field_error === true && $lang_field_error === true)
		{
			...
			$this->logger->logInfo('Error creating Category: '.$cat_name." Enable 'Accented URL' in your SEO settings if your category name uses accented characters.");
	...
	private function updateExistingCombination($id_combination, $id_product, $raw_prod, $settings)
	{
		...
		if ($this->validateCombination($combination) && $combination->save())
		{
			...
			$this->logger->logInfo(sprintf('Updating default Combination, product (ID: %s)', $id_product));
	...
	private function validateCombinationArray($combination)
	{
		foreach (Combination::$definition['fields'] as $field => $data)
		{
			if (isset($combination[$field]))
				$combination[$field] = $this->autoValidate($combination[$field], $data);
		}
	...
	private function setCombinations(&$raw_prod, &$settings, &$product_id, &$combination_id, &$attribute_ids)
	{
		...
		elseif ($attribute_ids) //create new combination only if the attribute group name is set
		{
			...
			if ($settings['validate_product'] != 0)
				$combination = $this->validateCombinationArray($combination);
			...
			if ($combination_id)
			{
				...
				$product = new Product($product_id);
				$product->checkDefaultAttributes();
				$this->logger->logInfo(sprintf('Updating default Combination, product (ID: %s)', $product_id));
		...
		foreach ($languages as $lang)
		{
			...
			foreach ($subs as $key)
			{
				if (isset($raw_prod[$settings['field_names']['feature'][$lang][$key]]))
				{
					...
					{
						if (!isset($feat_array[$lang]))
							$feat_array[$lang] = array();
						$feat_array[$lang] = array_merge($feat_array[$lang], explode(',', $raw_prod[$settings['field_names']['feature'][$lang][$key]]));
						$complex_import = true;
					}
					else
					{
		...
		if ($complex_import)
		{
			...
			foreach ($languages as $lang)
			{
				foreach ($feat_array[$lang] as $feature_string)
				{
					...
					$_feature[0] = preg_replace('/(_)/', ' ', $_feature[0]);
					//search for attribute group id in all set store languages
	...
	private function simpleFeatureImport($raw_prod, $settings, $product_id, $total_feat)
	{
		...
		foreach ($total_feat as $field)
		{
			...
			{
				//search for attribute group id in all set store languages
				$_feature_name = preg_replace('/(_)/', ' ', $feature_name);
				if ($feature_id = $this->getFeatureId($_feature_name, $lang))
				{
			...
			if (!isset($feature_id))
			{
				...
				$feature = new Feature();
				foreach ($field as $lang => $feature_name)
					$feature->name[$lang] = $_feature_name;
	...
	private function setProductDownload(&$raw_prod, &$settings, &$product_id)
	{
		...
		if (file_exists(realpath(_PS_DOWNLOAD_DIR_).'/'.$file))
		{
			...
			$this->logger->logInfo('This file doesn\'nt exist: '.$raw_prod[$settings['field_names']['file']].' Please make sure your file for Downloading is in the correct directory. It should be in '.realpath(_PS_DOWNLOAD_DIR_));
	...
	*/
	private function setSpecificPrices(&$raw_prod, &$settings, &$product_id, &$combination_id = 0, &$simple = 0)
	{
		...
		//specify field group
		$field_group = $settings['field_names'];
					...
					if (isset($discounts[0]))
					{
						foreach ($discounts as $discount)
						{
							...
							if (count($discount_parts) >= 5)
							{
								...
								$specific_price->reduction = ($default_values['reduction_type'] == 'amount' && $discount_parts[0] != 0) ? $discount_parts[0]
								...
								//check is this specific price exists
								$price_exists = $this->doesSpecificPriceExist($specific_price, $product_id);
								$field_error = $specific_price->validateFields($this->validate);
								$lang_field_error = $specific_price->validateFieldsLang($this->validate);

								if ($field_error === true && $lang_field_error === true && $price_exists === 0)
									$specific_price->add();
	...
	/**
	 * Check whether SpecificPrice exists
	 *
	 * @param $specific_price SpecificPrice A specific_price object to check before adding
	 * @param $product_id     int
	 * @return boolean
	 */
	public function doesSpecificPriceExist($specific_price, $product_id)
	{
		return SpecificPrice::exists($product_id, $specific_price->id_product_attribute,
			$specific_price->id_shop, $specific_price->id_group, $specific_price->id_country,
			$specific_price->id_currency, $specific_price->id_customer, $specific_price->from_quantity,
			$specific_price->from, $specific_price->to
		);
	}
	...
	*/
	...
	{
		$name = $this->validateHeaderForSql($name);
		if (strcmp($name, $this->product_tag) == 0)
			$this->xml_product = array();
		//Get attributes
		foreach ($attr as $key => $value)
		{
			$key = $this->validateHeaderForSql($key);
	...
	public function createEmptyTable($headings)
	{
		...
		$sql = 'CREATE TABLE IF NOT EXISTS '._DB_PREFIX_.'hj_import (hj_id INT(11) AUTO_INCREMENT, ';
	...
	public function fetchAttachment($attachment_source)
	{
		...
		if (file_exists(realpath(_PS_DOWNLOAD_DIR_).'/'.$attachment_source))
		{
			$new_name = sha1(microtime());
			$mimetype = mime_content_type(realpath(_PS_ROOT_DIR_).'/'.$attachment_source);
			if (rename(realpath(_PS_DOWNLOAD_DIR_).'/'.$attachment_source, realpath(_PS_DOWNLOAD_DIR_).'/'.$new_name))
			{
	...
	/**
	 * Determines whether or not a URL is remote or not
	 *
	 * @param (string) $source - source url
	 * @return (boolean) isRemote
	 */
	public function isImageSourceRemote($source)
	{
		$url = preg_replace('/\s/', '%20', $source);
	 ...
	 */
	public function getValidImage($image_source, $shouldFetchRemote)
	{
		$image_source = rtrim(trim($image_source), '/'); //remove spaces and newlines around source and any leading /
		$isRemoteURL = $this->isImageSourceRemote($image_source);
		$image_types = array('png', 'jpeg', 'jpg', 'gif');

		$valid_image = false;
		if ($isRemoteURL && $shouldFetchRemote)
		{
			$image_source = preg_replace('/\s/', '%20', $image_source);
		...
		else
		{
			foreach ($image_types as $type)
			{
				if (strstr($image_source, $type) !== false)
				{
					...
					$file_path_root = _PS_ROOT_DIR_.'/'.$image_source;
					$file_path_img  = _PS_IMG_DIR_.$image_source;
					if (file_exists($file_path_root))
						$valid_image = $file_path_root;
					else
					{
						if (file_exists($file_path_img))
							$valid_image = $file_path_img;
						else
							$this->logger->logWarning($file_path_root.' and '.$file_path_img.' do not exist, skipping image.');
					}
					break;
				}
			}
	...
	*/
	...
	{
		if (strstr($image_source, '^') === false)
			$image_sources = array($image_source);
		else
			$image_sources = explode('^', $image_source);
	...
	/**
	 * Gets active carrier by name
	 * @param string $name Name of carrier to fetch
	 */
	...
	{
		list($tmp_width, $tmp_height, $type) = getimagesize($file);
	...
	{
		$sql1 = 'CREATE TABLE IF NOT EXISTS '._DB_PREFIX_.'hj_import (hj_id INT(11) AUTO_INCREMENT, PRIMARY KEY (hj_id))';
		$sql2 = 'CREATE TABLE IF NOT EXISTS '._DB_PREFIX_.'hj_existing_prods (product_id INT(11) AUTO_INCREMENT, product_id_field VARCHAR(64), PRIMARY KEY (product_id))';
		$sql3 = '
	...
	private function deleteImages($product_id, $image_id = 0)
	{
		...
		$this->logger->logInfo('Images deleted for product ID: '.$product_id);
		// update positions
		$result = Db::getInstance()->executeS('
			SELECT *
			FROM `'._DB_PREFIX_.'image`
	...
	/*
	 * Checks the 'source' setting from step 1, if it's not set to file upload then a profile is compatible for cron
	 */
	private function profileSuitableForCron($profile_name)
	{
		$cron_suitable = false;

		$query = 'SELECT `value` FROM `'._DB_PREFIX_.'hj_import_settings` WHERE `step` = \'1\' AND `name` = \'source\' AND `group` = \''.bqSQL($profile_name).'\'';
		if ($result = Db::getInstance()->executeS($query))
		{
			if (isset($result[0]['value']))
			{
				$source = unserialize($result[0]['value']);
				$cron_suitable = ($source != 'feed_file');
			}
		}

		return $cron_suitable;
	}
	...
	private function zeroQuantityProduct($product_id)
	{
		if (version_compare(_PS_VERSION_, '1.6', '<'))
		{
			...
			Db::getInstance()->execute('UPDATE '._DB_PREFIX_.'stock_available SET `quantity` = 0 WHERE `id_product` = '.(int)$product_id);
	}
	...
	{
		$remove_text = htmlentities($adjustment[0], ENT_COMPAT, 'UTF-8');
		$field = $adjustment[1];
	...
	{
		$str = htmlentities($adjustment[0], ENT_COMPAT, 'UTF-8');
		$replacement = htmlentities($adjustment[1], ENT_COMPAT, 'UTF-8');
		$field = $adjustment[2];
	...
	/**
	 * @param (mixed) array(field to adjust, division factor)
	 */
	public function divide($adjustment)
	{
		$field = $adjustment[0];
		$divisor = $adjustment[1];
		if ($divisor == '0')
			$divisor = 1;
		Db::getInstance()->execute('UPDATE '._DB_PREFIX_.'hj_import SET `'.$field.'` = (`'.$field.'` / '.(float)$divisor.' )');
	}
	...
	{
		$field = $adjustment[0];
		$value = htmlentities($adjustment[1], ENT_COMPAT, 'UTF-8');
	...
	{
		$field1 = $adjustment[0];
		$separator = htmlentities($adjustment[1]);
		...
		$new_columns = ($query[0]['new_columns'] / Tools::strlen($separator)) + 1;
	...
	{
		if (!($bool === null || is_bool($bool) || preg_match('/^(0|1)$/', $bool)))
		{
			$this->logger->logInfo('Bool value invalid, making valid by setting to false.');
			$bool = false;
		}
	...
	public function isCatalogName($name)
	{
		...
		$this->logger->logInfo('Catalog name value invalid, making valid by setting to '.$name);
		return $name;
	}
	...
	public function isCleanHtml($html)
	{
		...
		$html = 'empty';
		$this->logger->logInfo('HTML value invalid, making valid by setting to '.$html);
		return $html;
	}
	...
	public function isDateFormat($date)
	{
		...
		$this->logger->logInfo('Date value invalid, making valid by setting to '.$date);
		return $date;
	}
	...
	public function isGenericName($name)
	{
		...
		foreach ($special_chars as $char)
			$name = str_replace($char, '', $name);
		$this->logger->logInfo('Name value invalid, making valid by setting to '.$name);
	 ...
	 * Make ean13 valid
	 *
	 * @param string $ean13 Barcode to make valid
	 * @return string
	 */
	public function isEan13($ean13)
	{
		$ean13 = preg_replace('/[^0-9]/i', '', $ean13);
		if (Tools::strlen($ean13) > 13)
			$ean13 = Tools::substr($ean13, 0, 13);
		$this->logger->logInfo('EAN13 value invalid, making valid by setting to '.$ean13);
		return $ean13;
	}
	...
	{
		return $this->str2url($link);
	}

	/**
	 * Function is ripped from Tools::str2url
	 *
	 * There was a bug with accented characters that needed fixing, I've submitted
	 * a pull request to the PrestaShop repo but in the meantime we need to use
	 * this cloned function for our imports.
	 */
	...
	{
		$price = preg_replace('/[^0-9.]/i', '', $price); //leave only 0-9 and .
		if (Tools::strlen($price) < 1)
			$price = 0.0;
		else
			$price = number_format((float)$price, 8, '.', '');
		$this->logger->logInfo('Price value invalid, making valid by setting to '.$price);
		return $price;
	}
	 ...
	 *
	 * @param string $visibility visibility to make valid
	 * @return string
	 */
	public function isProductVisibility($visibility)
	{
		...
		$visibility = 'both';
		$this->logger->logInfo('Visiblilty value invalid, making valid by setting to '.$visibility);
		return $visibility;
	}
	...
	{
		$reference = preg_replace('/[<>;=#{}]/i', '', $reference);
		$this->logger->logInfo('Reference value invalid, making valid by setting to '.$reference);
		return $reference;
	}

	/**
	 * Validate a non string
	 *
	 * @param string $data Data to string validate
	 * @return string
	 */
	public function isString($data)
	{
		$this->logger->logInfo('String value invalid, making valid by setting to ""');
		return '';
	}
	...
	public function isUnsignedInt($value)
	{
		...
		if ($value >= 4294967296)
			$value = 0;
		$this->logger->logInfo('Int value invalid, making valid by setting to '.$value);
		return $value;
	}
	...
	{
		//remove all non [0-9] chars
		$upc = preg_replace('/[^0-9]/i', '', $upc);
		//ensure length < 12
		if (Tools::strlen($upc) > 12)
			$upc = Tools::substr($upc, 0, 12);
		$this->logger->logInfo('UPC value invalid, making valid by setting to '.$upc);
		return $upc;
	}
}
