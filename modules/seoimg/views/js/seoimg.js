...
var Main = function () {
	...
	*/
	var loadFormModal = function (id, role, type, table) {
		...
		role = (typeof(role) !== 'undefined') ? role : 'url';
					...
					onshow: function(dialogRef) {
						...
						var checkStep = function (obj, context) {
							...
							if (((current_step === 2 && next_step === 3) || (next_step >= 3 || current_step > 3)) && role === 'meta') {
								if (next_step === 3) {
									checkInputs($body.find('#legend'), $next_button);
								}
							}
							...
							if ((current_step === 2 && next_step === 3) || current_step === 1 && next_step === 3) {
								$footer.find("#next-step").addClass('hide');
							}

							// Force first input focus
							if ($body.find('#legend').length) {
								$body.find('#legend').focus();
							}
